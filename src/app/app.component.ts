...
export class AppComponent {
  ...
  GetResponse() {
    this.url = 'http://' + this.localhost + ':8080/events7/data.jsonx?a=' + this.user.compte + '&p=' + this.user.motDePass + '&u=' + this.user.login + '&g=all&l=1&at=true';
  ...
  expanded(DeviceId: string, DeviceDesc: string = "") {
    ...
    if (this.ExistDeviceID != DeviceId) {
      ...
      this.url = 'http://' + this.localhost + ':8080/eventsApp2/data.jsonx?a=' + this.user.compte + '&p=' + this.user.motDePass + '&u=' + this.user.login + '&d=' + DeviceId + '&rf=1625443200&rt=1625529599&l=5000&at=true';
          ...
          this.DeviceList1.forEach(deviceData1 => {
            if (deviceData1.EventData.length) {
              deviceData1.EventData.forEach((eventDataItem, i) => {
                ...
                if (eventDataItem.StatusCode == 62465 || eventDataItem.StatusCode == 61714) {
                  if (!this.isStart) {
                ...
                if (this.isStart && this.isFinal) {
                  ...
                  this.tableModel.duration = this.convertHMS(this.final.Timestamp - this.start.Timestamp);
                  this.tableModel.distance = this.final.Odometer - this.start.Odometer;
  ...
  validateTop(event: ResizeEvent): boolean {
    ...
    if (event.rectangle.height < MIN_DIMENSIONS_PX || event.rectangle.height > MAX_DIMENSIONS_PX) {
      return false;
    }
    return true;
  }
